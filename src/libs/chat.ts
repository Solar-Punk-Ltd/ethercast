...
export interface User {
  // User object, that will be uploded to Users feed, at registration
  username: string;
  address: EthAddress;
  timestamp: number;
  signature: Signature;
...
export interface UserWithIndex extends User {
  // Same as User object, but contains an index for the user's feed
  index: number;
}
...
export async function initChatRoom(topic: string, stamp: BatchId): Promise<{ usersRef: Reference } | null> {
  try {
...
async function createUsersFeed(topic: string, stamp: BatchId) {
  try {
    ...
    const exist = await bee.isFeedRetrievable('sequence', graffitiSigner.address, consensusHash);
...
export async function registerUser(topic: string, streamerAddress: EthAddress, username: string, stamp: BatchId) {
  try {
    ...
    do {
      // Try to upload User object, if not successful, try again
      try {
...
export async function writeToOwnFeed(
  ...
  try {
    ...
    console.error(
      `There was an error while trying to write own feed (chat), index: ${index}, message: ${messageObj.message}: `,
      error,
    );
    return null;
  }
...
export async function uploadObjectToBee(jsObject: object, stamp: BatchId): Promise<UploadResult | null> {
  try {
