...
async function initChatRoom(topic, privKey, stamp) {
    try {
...
async function registerUser(topic, streamerAddress, username, stamp) {
    try {
            ...
            try {
                ...
                for (let i = 0; i < MAX_TRY_ATTEMPT; i++) {
                    try {
                        ...
                        await (0, common_1.sleep)(10 * 1000);
                    }
                }
            }
            catch (error) {
...
async function writeToOwnFeed(topic, streamerAddress, index, messageObj, stamp) {
    try {
...
async function fetchAllMessages(userList, streamTopic) {
    try {
...
// This is called on the side of the Streamer (aggregator)
async function writeOneMessageToAggregatedFeed(message, chatWriter, chatIndex, stamp) {
    try {
...
// This is called on the side of the Streamer (aggregator)
async function updateUserList(topic, index = 0, users = []) {
    try {
        const roomId = (0, chat_1.generateUsersFeedId)(topic);
...
async function uploadObjectToBee(jsObject, stamp) {
    try {
        const result = await bee.uploadData(stamp, (0, graffitiUtils_1.serializeGraffitiRecord)(jsObject), { redundancyLevel: 4 });
        return result;
    }
    catch (error) {
        return null;
...
async function readSingleMessage(index, streamTopic, streamerAddress) {
    try {
...
async function getGraffitiFeedIndex(roomId) {
    try {
