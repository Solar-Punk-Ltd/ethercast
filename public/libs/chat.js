...
import { getConsensualPrivateKey, getGraffitiWallet, serializeGraffitiRecord, } from '../utils/graffitiUtils';
...
export async function initChatRoom(topic, privKey, stamp) {
    try {
...
async function createUsersFeed(topic, stamp) {
    try {
...
async function createAggregatedFeedWriter(streamTopic, wallet) {
    try {
...
export async function registerUser(topic, streamerAddress, username, stamp) {
    try {
        ...
        localStorage.setItem(generateUniqId(topic, streamerAddress), address);
...
export async function writeToOwnFeed(topic, streamerAddress, index, messageObj, stamp) {
    try {
        const address = localStorage.getItem(generateUniqId(topic, streamerAddress));
        if (!address)
        ...
        const newRef = bytesToHex(newChunk.address());
...
export async function fetchAllMessages(userList, streamTopic) {
    try {
        // List of promises, that will give back { user, messages }, if successful
        const promiseList = userList.map(async (user) => {
            const messages = [];
...
// This is called on the side of the Streamer (aggregator)
export async function writeOneMessageToAggregatedFeed(message, chatWriter, chatIndex, stamp) {
    try {
...
// This is called on the side of the Streamer (aggregator)
export async function updateUserList(topic, index = 0, users = []) {
    try {
        const roomId = generateUsersFeedId(topic);
...
export function feedWriterFromRoomId(roomId) {
...
export function feedReaderFromRoomId(roomId) {
    const privateKey = getConsensualPrivateKey(roomId);
    const wallet = getGraffitiWallet(privateKey);
    const graffitiSigner = {
        address: Utils.hexToBytes(wallet.address.slice(2)), // convert hex string to Uint8Array
...
export async function uploadObjectToBee(jsObject, stamp) {
    try {
...
export async function readSingleMessage(index, streamTopic, streamerAddress) {
    try {
        ...
        const feedReader = bee.makeFeedReader('sequence', topic, streamerAddress, { timeout: 500 });
...
export async function getGraffitiFeedIndex(roomId) {
    try {
