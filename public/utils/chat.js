...
export function generateUserOwnedFeedId(topic, userAddress) {
    return `${topic}_EthercastChat_${userAddress}`;
}
// UniqID will contain streamer address + topic
export function generateUniqId(topic, streamerAddress) {
    return `${streamerAddress}-${topic}`;
}
// Used for message obj serialization
export function objectToUint8Array(jsObject) {
...
export function removeDuplicate(messages) {
